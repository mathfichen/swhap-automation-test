...
| **D12** | Raw-materials PII exception | `raw_materials/` is preserved **byte-identical, except for documented PII redaction.** When an archive bundles a third party's personal data unrelated to the software, the curator may repackage the raw archive to remove it, recorded as a journaled curator action (original noted). Rationale: the byte-identity invariant preserves *the software*, not a vehicle to publish unrelated PII. Precedent: C-Prolog #2 shipped a student's homework containing an SSN; both raw and curated trees were redacted. Ruled 2026-07-08. |
